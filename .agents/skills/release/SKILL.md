---
name: release
description: Run the pi-auto release flow end-to-end. Use when the user asks to release patch, minor, major, beta, or an explicit semver; includes non-interactive release execution, GitHub Actions monitoring with gh, npm trusted publishing verification, draft release note cleanup, and publishing the GitHub Release.
---
...
```bash
TAG="v0.0.0" # replace
npm view @kimuson/pi-auto version
...
```bash
npm view @kimuson/pi-auto dist-tags --json
```
...
```bash
RELEASE_URL="https://github.com/d-kimuson/pi-auto/releases/tag/v0.0.0" # replace
...
```bash
TAG="v0.0.0" # replace
NOTES_FILE="/tmp/pi-auto-$TAG-release-notes.md"
